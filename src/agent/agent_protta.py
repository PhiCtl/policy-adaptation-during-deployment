...
def make_agent(obs_shape, action_shape, args):
    return SacSSAgent(
        ...
        args=args # TODO -> better practise
    )
...
class Actor(nn.Module):
    ...
    def __init__(
        ...
        self.input_feature_dim = self.encoder.feature_dim + latent_dim
        self.trunk = nn.Sequential(
            nn.Linear(self.input_feature_dim, hidden_dim), nn.ReLU(),
    ...
    def forward(
        self, obs, dynamics, compute_pi=True, compute_log_pi=True, detach_encoder=False
    ):
        obs = self.encoder(obs, detach=detach_encoder)
        latent_preds = self.latent_encoder(dynamics, detach=detach_encoder)

        input = torch.cat([obs, latent_preds], dim=1)
        mu, log_std = self.trunk(input).chunk(2, dim=-1)
...
class QFunction(nn.Module):
    """MLP for q-function."""
    def __init__(self, obs_dim, latent_dim, action_dim, hidden_dim):
        super().__init__()

        self.trunk = nn.Sequential(
            nn.Linear(obs_dim + action_dim + latent_dim, hidden_dim), nn.ReLU(),
            nn.Linear(hidden_dim, hidden_dim), nn.ReLU(),
            nn.Linear(hidden_dim, 1)
        )

    def forward(self, obs, latent_preds, action):
        assert obs.size(0) == action.size(0)

        obs_action_preds = torch.cat([obs, latent_preds, action], dim=1)
        return self.trunk(obs_action_preds)


class RotFunction(nn.Module):
    # TODO : modify with latent preds ?
...
class InvFunction(nn.Module):
    ...
    def __init__(self, obs_dim, latent_dim, action_dim, hidden_dim):
        ...
        self.trunk = nn.Sequential(
            nn.Linear(2*obs_dim + latent_dim, hidden_dim), nn.ReLU(),
            nn.Linear(hidden_dim, hidden_dim), nn.ReLU(),
            nn.Linear(hidden_dim, action_dim)
        )

    def forward(self, h, h_next, latent):
        joint_h = torch.cat([h, h_next, latent], dim=1)
...
class Critic(nn.Module):
    ...
    def __init__(
        ...
        self.Q1 = QFunction(
            self.encoder.feature_dim, latent_dim, action_shape[0], hidden_dim
        )
        self.Q2 = QFunction(
            self.encoder.feature_dim, latent_dim, action_shape[0], hidden_dim
        )
    ...
    def forward(self, obs, dynamics, action, detach_encoder=False):
        # detach_encoder allows to stop gradient propagation to encoder
        obs = self.encoder(obs, detach=detach_encoder)
        latent_preds = self.latent_encoder(dynamics, detach=detach_encoder)
        q1 = self.Q1(obs, latent_preds, action)
        q2 = self.Q2(obs, latent_preds, action)
...
class SacSSAgent(object):
    ...
    def __init__(
        ...
        if use_rot or use_inv:
            ...
            self.ss_latent_encoder = make_latent_encoder(
                dynamics_shape, latent_dim)
    ...
    def select_action(self, obs):
        with torch.no_grad():
            ...
            return mu.cpu().data.numpy().flatten(), dynamics
    ...
    def update_critic(self, obs, dynamics, action, reward, next_obs, not_done, L, step):
        ...
        with torch.no_grad():
            _, policy_action, log_pi, _ = self.actor(next_obs, dynamics)
            target_Q1, target_Q2 = self.critic_target(next_obs, dynamics, policy_action)
            target_V = torch.min(target_Q1,
                                 target_Q2) - self.alpha.detach() * log_pi
            target_Q = reward + (not_done * self.discount * target_V)

        # get current Q estimates
        current_Q1, current_Q2 = self.critic(obs, dynamics, action)
    ...
    def update_actor_and_alpha(self, obs, dynamics, L=None, step=None, update_alpha=True):

        # detach encoder, so we don't update it with the actor loss
        _, pi, log_pi, log_std = self.actor(obs, dynamics, detach_encoder=True)
        actor_Q1, actor_Q2 = self.critic(obs, dynamics, pi, detach_encoder=True)
