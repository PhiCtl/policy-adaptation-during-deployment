...
def make_domain_generic_agent(obs_shape, action_shape, args):
    return SacSSAgent(
        obs_shape=obs_shape,
...
class InvFunction(nn.Module):
    ...
    def forward(self, h, h_next):
        joint_input = torch.cat([h, h_next], dim=1)
...
class SacSSAgent(object):
    ...
    def predict_action(self, obs, next_obs, gt, L=None, step=None):
        """Make the forward pass for actor and ss head"""
