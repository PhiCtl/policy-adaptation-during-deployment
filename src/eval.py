...
def evaluate(env, agent, args, video=None, recorder=None, adapt=False, reload=False, exp_type=""):
    ...
    for i in tqdm(range(args.pad_num_episodes)):
        ...
        while not done:
            ...
            # If we want to reload the initial pre-trained weight to mitigate catastrophic forgetting
            # TODO a bit artificial though
...
def main(args):
    ...
    if args.use_inv or args.use_curl or args.use_rot:
        ...
        pad_reward, std = evaluate(env, agent, args, video = video, recorder = recorder, adapt=True, exp_type="pad", reload=True)
